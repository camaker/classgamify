...
3. **Enable storage in website config** (`src/config/website.ts`):
   ...
   card are active. Teacher-managed links use `/api/storage/file?id=...`;
   public shared-folder URLs may use `/api/storage/file?key=...`.
...
Files are always served via the same-origin storage route. Teacher-managed file
links use `/api/storage/file?id=...` so the browser carries only the stable file
id and the route resolves private R2 keys on the server. The `?key=...` form
remains available for public shared-folder assets generated by the provider.
...
- **resolveStorageFileAccessDecision** and
  **buildStorageFileResponseHeaders** (shared helpers, in
  ...
  contract for the same-origin storage route, including key validation, public
...
- **uploadUserFile** (server function, in `@/api/user-files`): Accepts
  ...
  - The server function sanitizes provider output before returning it. Private
    uploads return a safe file item without `key`, `r2Key`, owner id, description,
    or provider metadata. Public shared-folder uploads return only their public
    URL for avatar and product-asset consumers.
...
- **useUploadUserAvatar()** (client, in `@/hooks/use-user-files`): Mutation
  ...
  returns `{ url }`. Used by the avatar upload card.
...
- **GET /api/storage/file?id=...** and **GET /api/storage/file?key=...**
  - Streams the object from R2. ID links resolve private keys in D1 before the
    existing session and ownership check. Public shared folders such as `avatars`
    remain accessible by provider-generated key URLs. Access decisions and
    response headers are delegated to
