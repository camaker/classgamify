...
**ClassGamify** — a teacher-facing, game-based classroom activity platform
inspired by Wordwall and Liveworksheets. Teachers create reusable structured
activities, render them through game/worksheet templates, publish assignment
links for students, and review attempts/results. AI-assisted activity creation
and template remixing build on this activity → assignment → attempt foundation.

The codebase started from the TanStarter SaaS template, so copied-template
surfaces should be retired in narrow, verified waves. Product behavior should
follow [docs/product.md](docs/product.md): keep classroom activity, assignment
link, student runner, and teacher result-review loops first-class.
...
| `src/api/` | Server functions (activities, assignments, AI drafts, payment, users, contact, newsletter, files) |
| `src/activities/` | Template catalog, content validation, runtime item generation, scaffolds, remix readiness, AI draft mapping |
| `src/assignments/` | Assignment settings, share links, lifecycle, public payloads, submission rules, scoring/results, exports, review summaries |
...
- `app.schema.ts` — application tables including activities, assignments, snapshots, attempts, user files, and payment records
