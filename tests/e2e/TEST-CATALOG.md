...
| 6 | Teacher can review, copy, and export assignment results | After at least one student attempt, open `/dashboard/assignments/:assignmentId`, verify completions, average accuracy, share link, copyable student link, the student summary table, the attempt table, reteach priorities, the full item performance table, per-item correct rates, answer explanations, and answer review details show scored student answers from the assignment snapshot, then download the CSV export and verify it includes student summary, attempt, and item-level answer columns. |
