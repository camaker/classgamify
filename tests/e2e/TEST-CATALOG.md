...
| 5 | Student runner adapts to template content | Create quiz, match-up, fill-blank, matching-pairs, and group-sort activities, publish each one, open each `/play/:shareId`, verify quiz and match-up render clickable choices, verify fill-blank renders worksheet-style inline blanks instead of multiple-choice cards, verify group-sort renders a category board where items move into selected groups, verify matching-pairs renders a left/right card board that records selected choices without exposing correct answers, submit answers, and verify scoring reflects the template-specific expected answers. |
