...
| 22 | Teacher can search and filter the activity library | Sign in, create multiple saved activities with distinct titles, descriptions, and template types, open `/dashboard/activities`, search by a title keyword, verify only matching activity cards remain and the URL keeps `q`, search by a description or template keyword, verify matching cards update, filter by an exact template family such as quiz or group sort, verify the URL keeps `template`, combine it with search or archived status, clear filters, and verify the full library returns. |
