...
| 17 | Homepage and template routes enter ClassGamify product loops | Open `/`, verify the hero preview and primary actions point to templates or activity creation rather than legacy Hanzi, HSK, or worksheet routes, then follow the primary CTA and verify the activity creation page loads. Open `/templates`, verify the template cards describe real classroom modes and content requirements, click a template-specific start action, and verify `/create?template=...` loads with that primary template selected. Open `/roadmap` and `/dashboard`, verify they describe the current usable create-publish-play-results loop rather than stale skeleton-only milestones. |
...
| 22 | Teacher can search, filter, and page the activity library | Sign in, create enough saved activities to exceed one library page with distinct titles, descriptions, and template types, open `/dashboard/activities`, verify the result range and next/previous controls, move to page 2 and verify the URL keeps `page`, search by a title keyword and verify the list resets to page 1 with `q`, search by a description or template keyword, verify matching cards update, filter by an exact template family such as quiz or group sort and verify the URL keeps `template`, combine it with search or archived status, clear filters, and verify the full library returns. |
