...
| 15 | Time-limited assignments show attempt duration | Publish an assignment with a short time limit, verify `/dashboard/assignments` shows the timer, open `/play/:shareId`, verify the countdown appears, submit an attempt, verify the score panel shows elapsed time, and verify `/dashboard/assignments/:assignmentId` shows average time plus per-attempt duration. |
