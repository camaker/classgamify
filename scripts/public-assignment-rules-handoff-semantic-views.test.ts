...
test('public assignment rules expose a safe 20-slice handoff view', () => {
  ...
  assert.match(handoffView.description, /20-slice public assignment rules/);
...
test('public assignment rules handoff localizes Chinese boundary values', () => {
  ...
  try {
    ...
    assert.match(handoffView.description, /20 切片公开作业规则交接/);
    assert.equal(
...
test('public assignment rules handoff contract is documented', () => {
  assert.match(
    ...
    'Public assignment rules should expose the full 20-slice handoff id contract.'
  );
  assert.match(
    TEST_CATALOG_SOURCE,
